...
cannot discover a source version while installing Reflow in an image. Build
workflows use this helper on the host and pass the normalized result through
``REFLOW_BUILD_VERSION`` without copying repository metadata into an image.
"""
...
def version_from_git_description(
    ...
    *,
    ...
    Returns:
        A normalized version suitable for setuptools-scm's pretend-version
        environment variable.

    Notes:
        SemVer prerelease tags such as ``v1.0.0-rc.1`` normalize to PEP 440
        package versions such as ``1.0.0rc1``. The Git tag itself is unchanged.
    """
    ...
    if tagged_version.dev is not None:
        next_dev = tagged_version.dev + distance
        return f"{tagged_version.base_version}.dev{next_dev}"

    if tagged_version.post is not None:
        next_post = tagged_version.post + 1
        return f"{tagged_version.base_version}.post{next_post}.dev{distance}"
...
def resolve_repository_version(
    ...
    *,
    ...
    """Resolve a build version from the nearest repository tag.
